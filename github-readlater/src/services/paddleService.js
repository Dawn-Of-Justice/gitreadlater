      ...
      case 'subscription.created': {
        ...
        if (userId) {
          ...
          const result = await updateUserSubscription(
            ...
            validUntil,
            subscription.id,
            subscription.customer_id // Store customer_id in paddle_user_id field
          );
        ...
        if (userId) {
          ...
          if (transaction.subscription && transaction.subscription.next_billed_at) {
            ...
            const result = await updateUserSubscription(
              ...
              transaction.customer_id // Store customer_id in paddle_user_id field
            );
...
app.post('/customer-portal', async (req, res) => {
  try {
    ...
    const { data: subscription, error: subError } = await supabase
      .from('user_subscriptions')
      .select('paddle_user_id, paddle_subscription_id')
      .eq('user_id', userId)
      .single();
    ...
    if (!subscription?.paddle_user_id) {
      console.log('Error: No Paddle customer ID found');
      return res.status(400).json({ error: 'No active subscription found' });
    }
    
    console.log('Customer ID found:', subscription.paddle_user_id);
    ...
    const response = await axios.post(
      ...
      {
        customer_id: subscription.paddle_user_id // Using paddle_user_id as the customer_id
      },
      {
        headers: {
          'Content-Type': 'application/json',
